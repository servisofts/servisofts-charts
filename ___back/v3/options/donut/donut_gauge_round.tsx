    ...
    render() {
        ...
        const render_circles = () => {
            ...
            return arr.map((obj, i) => {
                ...
                const p = <Path d={Func.describeDonutPathRound(center, center, grosor + x, 0, angle, grosor, rx)}
                    fill={obj.color}
                    stroke={obj.color}
                    {...this.props.style}
                    {...obj.style}
                />
                const pBack = <Path d={Func.describeDonutPathRound(center, center, grosor + x, 0, 360 * 0.75, grosor, rx)}
                    fill={obj.color}
                    opacity={0.1}
                />
                currentAngle += angle;
